...
def main():
    modo = sys.argv[1] if len(sys.argv) > 1 else "file"

    if modo == "file":
        archivo = getfile("Seleccionar archivo", "Texto (*.txt);;Python (*.py)", multi=False)
        print("🗂️ Archivo seleccionado:", archivo)

    elif modo == "multi":
        archivos = getfile("Seleccionar múltiples archivos", "Todos (*)", multi=True)
        print("📂 Archivos:", archivos)

    elif modo == "folder":
        carpeta = getfolder("Seleccionar carpeta")
        print("📁 Carpeta:", carpeta)

    elif modo == "save":
        guardar = savefile("Guardar como", "Texto (*.txt)")
        print("💾 Guardar en:", guardar)

    else:
        print("❌ Modo no reconocido. Usar: file | multi | folder | save")
